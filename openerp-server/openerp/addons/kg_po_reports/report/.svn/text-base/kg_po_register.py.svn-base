	...
	def get_data(self,form):
		...
		if not form['status'] or form['status'] == 'approved' or form['status'] == 'cancelled':	
			self.cr.execute('''
				  ...
				  where po.state=%s and po.date_order >=%s and po.date_order <=%s '''+ product + partner + '''
			...
			self.cr.execute('''
				  ...
				  where po.state='approved' and pol.pending_qty > 0 and po.date_order >=%s and po.date_order <=%s '''+ product + partner + '''
